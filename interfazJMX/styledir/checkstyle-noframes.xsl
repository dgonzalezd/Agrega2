...
<!-- Modificado por David Recio (Enero 08) para sacer un resumen-->
    ...
    <xsl:key name="violations" match="file/error" use="@source"/>
	  ...
	  <xsl:template match="checkstyle" mode="summary">
        ...
        <xsl:variable name="fileCount" select="count(file)"/>
		    ...
		    <xsl:variable name="percent" select="round($errorCount div $fileCount)"/>
		    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
		    <tr>
			     <th>Archivos</th>
			     <th>Violaciones</th>
			     <th>Violaciones por archivo</th>
		    </tr>
		    <tr>
		      <xsl:call-template name="alternated-row"/>
            ...
            <xsl:for-each select="file/error[@source and generate-id(.) = generate-id(key('violations', @source))]">
                ...
                <tr>
                    ...
                    <td>
                        <xsl:value-of select="round (count(/checkstyle/file/error[@source=$source]) div $fileCount)"/>
                    </td>
                </tr>
            </xsl:for-each>
        </table>
